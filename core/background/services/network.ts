...
export class NetworkControl {
  ...
  public async sync() {
    const data = await BrowserStorage.get(Fields.CONFIG, Fields.SELECTED_NET);
  ...
  public async changeNetwork(selected: string) {
    ...
    await BrowserStorage.set(buildObject(Fields.SELECTED_NET, selected));
  ...
  public async changeConfig(config: typeof NETWORK) {
    await BrowserStorage.set(buildObject(Fields.CONFIG, config));
