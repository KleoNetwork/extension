...
export class TransactionsController {
  ...
  get #transactionsField() {
    if (this.#account.selectedAccount) {
      return `${Fields.TRANSACTIONS}/${this.#network.selected}/${
        this.#account.selectedAccount.base16
      }`;
    }
  ...
  public async clearConfirm() {
    ...
    await BrowserStorage.set(buildObject(this.#confirmField, this.forConfirm));
  ...
  public async addMessage(message: MessagePayload) {
    ...
    await BrowserStorage.set(buildObject(Fields.CONFIRM_MESSAGE, this.message));
  }
