...
export class ZRC2Controller {
  ...
  public async getAllowances(contract: string, token: string, owner: string) {
    ...
    const identities = [
      this.#zilliqa.provider.buildBody(Methods.GetSmartContractSubState, [
        tohexString(token),
  ...
  public async getBalance(owner: string) {
    ...
    const poolIdentities = onlyZRC2.map((token) =>
      this.#zilliqa.provider.buildBody(Methods.GetSmartContractSubState, [
        tohexString(this.#dex.contract),
        ZRC2Fields.Pools,
        [token.base16.toLowerCase()],
      ])
    );
    ...
    const entries = replies
      .slice(0, balanceIdentities.length)
      .map((res: RPCResponse, index: number) => {
  ...
  #toZRC2(init: InitItem[]) {
    ...
    assert(
      ...
      `${InitFields.ContractOwner} ${ErrorMessages.RequiredParam}`
    );
    assert(Boolean(name), `${InitFields.Name} ${ErrorMessages.RequiredParam}`);
    assert(
      Boolean(symbol),
      `${InitFields.Symbol} ${ErrorMessages.RequiredParam}`
    );
    assert(
      Boolean(address),
      `${InitFields.Address} ${ErrorMessages.RequiredParam}`
    );
