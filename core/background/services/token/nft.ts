...
export class NFTController {
  ...
  public get field() {
    assert(
      Boolean(this.#account.selectedAccount),
      ErrorMessages.Base16NotValid
    );
  ...
  public async fetchBatch(addresses: string[]): Promise<ZRCNFT[]> {
    ...
    const identities = addresses
      .map((addr) => {
        const base16 = tohexString(addr);
        return [
          this.#zilliqa.provider.buildBody(Methods.GetSmartContractInit, [
            base16,
          ]),
      ...
      .flat();
    const responses: RPCResponse[] = await this.#zilliqa.sendJson(
      ...identities
    );
    const list = chunk(responses, 4).map(
      async ([init, tokenOwners, tokenUrls, baseUri]) => {
