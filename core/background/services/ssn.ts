...
export class SSnController {
  ...
  public async update() {
    ...
    await BrowserStorage.set(buildObject(Fields.SSN, this.payload));
  }
  ...
  public async sync() {
    ...
    try {
      ...
      assert(TypeOf.isArray(list), `list ${ErrorMessages.MustBe} Array`);
      assert(!isNaN(selected), `selected ${ErrorMessages.RequiredParam}`);
