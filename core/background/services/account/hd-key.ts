...
export class HDKey {
  ...
  public deriveChild(index: number) {
    ...
    const I = Hmac(sha512, this.chainCode).update(data).digest();
    ...
    if (this.privateKey) {
      ...
      try {
        hd.privateKey = Uint8Array.from(
          secp256k1.privateKeyTweakAdd(this.privateKey, IL)
        );
      ...
      try {
        hd.publicKey = Buffer.from(
          secp256k1.publicKeyTweakAdd(this.publicKey, IL, true)
        );
  ...
  public fromMasterSeed(
    seedBuffer: Buffer,
    versions?: typeof BITCOIN_VERSIONS
  ) {
    const I = Hmac(sha512, MASTER_SECRET).update(seedBuffer).digest();
  ...
  #hash160(buf: Buffer) {
    ...
    return ripemd160().update(hash).digest();
  }
}
