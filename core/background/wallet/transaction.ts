...
import {
  StatusCodes,
  TransactionTypes,
} from "background/services/transactions";
...
export class ZilPayTransaction {
  ...
  public async addConfirm(params: MinParams, sendResponse: StreamResponse) {
    ...
    if (
      Number(params.gasPrice) >= this.#core.gas.gasPrice &&
      Number(params.gasPrice) < 9999
    ) {
  ...
  public async getRequiredParams(
    accIndex: number,
    ...
    try {
      ...
      const identities = [
        this.#core.zilliqa.provider.buildBody(Methods.GetNetworkId, []),
        this.#core.zilliqa.provider.buildBody(Methods.getBalance, [
          tohexString(account.base16),
        ]),
        this.#core.zilliqa.provider.buildBody(Methods.GetMinimumGasPrice, []),
      ];
      const [netIdRes, accountRes, gasRes] = await this.#core.zilliqa.sendJson(
        ...identities
      );
      const version = netIdRes.result;
      const currentNonce = Boolean(accountRes.result)
        ? accountRes.result.nonce
        : 0;
  ...
  public async signSendTx(
    txIndex: number,
    accIndex: number,
    params: TransactionForConfirm,
    sendResponse: StreamResponse
  ) {
    let token = {
      decimals: ZIL.decimals,
    ...
    try {
      ...
      const nonce = isNaN(params.nonce)
        ? await this.#core.nonceCounter.nextNonce(account)
        : params.nonce;
