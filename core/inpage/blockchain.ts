...
export class Blockchain {
  ...
  public getMinerInfo(dsBlockNumber: number) {
    assert(
      !isNaN(dsBlockNumber),
      `dsBlockNumber ${ErrorMessages.ShouldBeNumber}`
    );
   ...
   */
  public getSmartContractSubState(
    addr: string,
    variableName: string,
    indices = []
  ) {
    ...
    assert(
      ...
      `variableName ${ErrorMessages.RequiredParam}`
    );
  ...
  public getStateProof(addr: string, variableHash: string, txBlock: number) {
    assert(Boolean(addr), `addr ${ErrorMessages.RequiredParam}`);
    assert(
      Boolean(variableHash),
      `variableHash ${ErrorMessages.RequiredParam}`
    );
    assert(!isNaN(txBlock), `txBlock ${ErrorMessages.ShouldBeNumber}`);
    ...
    return this.#provider.send(RPCMethod.GetStateProof, address, fixedhash, [
      String(txBlock),
    ]);
  }
